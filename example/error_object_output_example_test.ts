...
function deepFunction(level: number): void {
  if (level > 0) {
...
const circularError = new Error("Circular reference error") as Error & { self?: Error };
circularError.self = circularError;

logger.error("Error with circular reference", circularError);

console.log("\n10. Non-Error object logged as error:");
logger.error("String as error", "This is just a string, not an Error object");
